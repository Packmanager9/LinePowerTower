...
document.addEventListener('keyup', (event) => {
 ...
 //   tutorial_canvas_context.scale(.1, .1);  // this scales the canvas
    ...
    class Gridbox{
        ...
        draw(){
            if(this.body.color != "pink"){

                if(this.building.color == "transparent"){
    ...
    let startbutton = new Rectangle ( 300, 0, 25, 50, "black")
    let acceptbutton = new Rectangle ( 200, 0, 50, 50, "green")
    let rejectbutton = new Rectangle ( 450, 0, 50, 50, "blue")
    


    let outline = new Rectangle ( 50,55, (12*50), (12*50), "black")
    let inline = new Rectangle ( 55, 60, 590, 590, "white")

    let xu = 50
    let yu = 55
    for(let u = 0; u<144; u++){
        let gridbox1 = new Gridbox(xu, yu)
        xu+=50
        if(xu >= 650){
            xu = 50
            yu+=50
    ...
    let home = new Circle(700, (80+6*50),20,"red")

    let beasts = []


    let beast = new Circle(0,  (80+6*50), 15, "black", 1, 0) 
    beast.health = 1000
    beast.maxhealth = 1000
    beast.healthbar = new Rectangle ( beast.x-beast.radius, beast.y-20, 5, 20, "green")

    beast.upbounce = false
    beast.downbounce = false

    beasts.push(beast)

    // beast = new Circle(100,  (80+6*50), 15, "black", .5, 0) 
    // beast.health = 1000
    // beast.maxhealth = 1000
    // beast.healthbar = new Rectangle ( beast.x-beast.radius, beast.y-20, 5, 20, "green")
    ...
    // beast = new Circle(200,  (80+6*50), 15, "black", .5, 0) 
    // beast.health = 1000
    // beast.maxhealth = 1000
    // beast.healthbar = new Rectangle ( beast.x-beast.radius, beast.y-20, 5, 20, "green")
    ...
    // beast = new Circle(300,  (80+6*50), 15, "black", .5, 0) 
    // beast.health = 1000
    // beast.maxhealth = 1000
    // beast.healthbar = new Rectangle ( beast.x-beast.radius, beast.y-20, 5, 20, "green")
...
// interval, fill this with game logic 
    window.setInterval(function(){ 
                    ...
                    if(beasts[b].health > 0){
                        ...
                        for(let g = 0; g< grids.length;g++){
                            ...
                            if(grids[g].building.color != "transparent" ){
                                ...
                                beasts[b].x+=25
                                if(squarecircleface(grids[g].body, beasts[b])){
                                    teleface = true
                                }
                                beasts[b].x-=25
                            }
                            ...
                            if(grids[g].building.color != "transparent" ){
                                ...
                                if(squarecirclehead(grids[g].body, beasts[b])){
                                    beasts[b].upbounce = false
                                    beasts[b].downbounce = true
                                    beasts[b].xmom =  1
                                    beasts[b].ymom = 0
                                    beasts[b].y +=1
                                }
                                if(squarecirclefeet(grids[g].body, beasts[b])){
                                    beasts[b].downbounce = false
                                    beasts[b].upbounce = true
                                    beasts[b].ymom = 0
                                    beasts[b].xmom = 1
                                    beasts[b].y-=1
                                }
                        ...
                        if(teleface == true){
                            ...
                            if(beasts[b].upbounce == true){
                                beasts[b].ymom = -1
                            }else if(beasts[b].downbounce == true){
                                beasts[b].ymom = 1
                            }else{
                                if(Math.random() < 0.5){
                                    beasts[b].ymom = 1
        ...
        beast = new Circle(0, (80+6*50), 15, "black", 1, 0) 
        beast.health = 1000
        beast.maxhealth = 1000
...
function squarecircleass(square, circle){
